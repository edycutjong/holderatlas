...
proves the lockfile), commit `chore(release): vX.Y.Z` (`release.yml` appends `[skip ci]` so the bump does not re-trigger the pipeline), push an annotated tag and publish a GitHub Release with
