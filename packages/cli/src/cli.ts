  ...
  a = await atlas(client, input, {
    ...
    onProgress: flags.has("--json")
      ...
      : (e) => {
          ...
          if (e.type === "holders") process.stderr.write(`${D}holders ${e.fetched}: ${e.custody} exchange custody · ${e.human} people · ${e.structural} pools/contracts → examining ${e.examined.custody} + ${e.examined.human}${X}\n`);
          if (e.type === "wallet" && flags.has("--explain")) {
            const r = e.row;
            const where = r.country ? (r.country === "global" ? `${D}global${X}` : `${G}${r.country}${X}`) : r.bucket === "untraced" ? `${D}no exchange trace${X}` : r.bucket === "unnamed" ? `${Y}unnamed${X}` : r.bucket === "error" ? `${R}${r.error}${X}` : `${Y}${r.entityLabel} (not in table)${X}`;
            process.stderr.write(`  ${r.kind.padEnd(7)} ${short(r.address).padEnd(14)} ${pct(r.share).padStart(6)}  ${(r.via ?? "-").padEnd(10)} ${where}  ${D}${r.calls} calls · ${r.credits} cr${X}\n`);
          }
        },
  });
} catch (e) {
...
for (const c of a.countries) console.log(`${G}${bar(c.share)}${X} ${c.code}  ${pct(c.share).padStart(6)}  ${D}${c.wallets} wallet${c.wallets === 1 ? "" : "s"} · ${c.exchanges.join(", ")}${X}`);
if (a.global.wallets) console.log(`${D}${bar(a.global.share)} --  ${pct(a.global.share).padStart(6)}  global exchanges, no location by design · ${a.global.exchanges.join(", ")}${X}`);
if (a.otherEntity.wallets) console.log(`${D}${bar(a.otherEntity.share)} ??  ${pct(a.otherEntity.share).padStart(6)}  entities not in exchanges.json · ${a.otherEntity.exchanges.join(", ")}${X}`);
if (a.unnamed.wallets) console.log(`${Y}${bar(a.unnamed.share)}${X} ${D}??  ${pct(a.unnamed.share).padStart(6)}  exchanges Nansen cannot name on ${a.chain} · ${a.unnamed.wallets} wallets${X}`);
if (a.untraced.wallets) console.log(`${D}${bar(a.untraced.share)} ··  ${pct(a.untraced.share).padStart(6)}  no exchange trace in 1 year · ${a.untraced.wallets} wallets${X}`);
...
console.log(`${D}${a.credits} credits · ${a.calls.length} calls (${hits} cached${client.oldestHit ? `, as of ${client.oldestHit.slice(0, 16).replace("T", " ")} UTC` : ""}) · ${(a.ms / 1000).toFixed(1)}s · atlas ${a.hash}${X}`);
if (flags.has("--explain")) {
  ...
  for (const [ep, e] of byEp) console.log(`${D}  ${ep.padEnd(40)} ${String(e.n).padStart(3)} calls  ${String(e.cr).padStart(4)} cr  ${e.cached} cached  ${(e.ms / Math.max(1, e.n - e.cached)).toFixed(0)} ms avg live${X}`);
}
