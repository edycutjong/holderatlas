...
![tests](https://img.shields.io/badge/tests-142%20passing-22c55e?style=flat)
...
| Metric | Value | Source |
|---|---|---|
| Tests | **142 tests** (`npm test`) — every label string seen live pinned to its key; the timeout path; offline replay = same hash | `packages/core/test/` |
...
```
packages/core     client · cache · nansen (zod) · labels + exchanges.json · atlas (the engine) · fixtures · test/ (142)
