...
[![Submission on X](https://img.shields.io/badge/𝕏_Submission-@edycutjong-000000?style=for-the-badge&labelColor=0a0e13)](https://x.com/edycutjong/status/2102175902331507198)
...
~118 credits per cold map, 0 on a cache hit. **Live Nansen calls accounted for in this repo: 2,065**, no run counted twice — 1,070 fetched live by `npm run seed` (each fixture's `live.calls`, which counts only uncached calls — `scripts/seed.ts`) + 110 in the PEPE hero run printed in [DEMO.md](DEMO.md) (the PEPE fixture holds those responses and reads them from cache, so its `live.calls` is 0) + 885 in the two `npm run bench` tables in [docs/BENCH.md](docs/BENCH.md) (444 + 441; the bench uses a fresh in-memory store, so none of its calls fed a fixture). The fixtures hold every raw response of their 12 runs byte-for-byte; the bench prints counts and hashes, not responses. The account-wide total is on Nansen's usage dashboard, where the 1,000-call gate is checked. Cached calls are labelled and never counted; failed calls are shown as "lookup failed", never guessed. **Every call is visible while it happens:** the page's right-hand rail streams each Nansen request as a row — pending → live (green) · cached (grey) · error (red) — with `POST endpoint`, the chain and wallet filter, the credits, the latency and the response sha256, and its counters equal the provenance drawer's totals exactly (same `Call` objects, nothing synthetic).
